package com.hxd.jewelry.demo2.view;
...
/**
 * @author Cazaea
 * @time 2018/4/16 13:18
 * @mail wistorm@sina.com
 */
public class SplashTimerView extends AppCompatTextView {
    ...
    public SplashTimerView(Context context) {
        super(context);
        init();
    }
     ...
     * @param attrs
     */
    public SplashTimerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
        ...
        if (typedArray != null) {
             ...
             * 圆实心的颜色属性
             */
            if (typedArray.hasValue(R.styleable.CountDownProgress_solidColor)) {
                //有这个属性索引 获取用户设置的颜色
                solidColor = typedArray.getColor(R.styleable.CountDownProgress_solidColor, 0);
            } else {
                //没有这个属性索引 默认是浅灰色
                solidColor = typedArray.getColor(R.styleable.CountDownProgress_solidColor, Color.parseColor("#D3D3D3"));
            }

            /**
             * 园边框颜色属性
             */
            if (typedArray.hasValue(R.styleable.CountDownProgress_frameColor)) {
                //有这个属性索引 获取用户设置的颜色
                frameColor = typedArray.getColor(R.styleable.CountDownProgress_frameColor, 0);
            } else {
                //没有这个属性索引 默认是深灰色
                frameColor = typedArray.getColor(R.styleable.CountDownProgress_frameColor, Color.parseColor("#A9A9A9"));
            }
    ...
    /**
     * 绘制过程
     * 注意：canvas在绘制过程中是一层层覆盖的。所以绘制顺序不要写反了。
     * 如果先绘制实心圆再绘制文字再绘制空心圆。这时你会发现文字是看不到了，被覆盖了。
     *
     * @param canvas
     */
    @Override
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(frameWidth);//设置空心线宽度
        mPaint.setColor(frameColor);
        canvas.drawCircle(mBounds.centerX(), mBounds.centerY(), radius - frameWidth, mPaint);
